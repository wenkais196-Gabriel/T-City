            ...
            if data.doors then
                ...
                for k, v in pairs(data.doors) do
                    if #(playerCoords - v.objCoords) < 30 then
						...
						if data.doorType == "doublesliding" or (data.doorType == "double" and data.autoOpen) then
                            ...
                            if not IsDoorRegisteredWithSystem(v.doorHash) then
                                ...
                                if data.locked then
									...
									if data.doorType == "doublesliding" or (data.doorType == "double" and data.autoOpen) then
										...
										DoorSystemSetAutomaticRate(v.doorHash, data.doorRate or 1.0, false, false)
									end
                                end
                            end
                        end
                        ...
                        if data.object and data.object ~= 0 then
                            ...
                            if not IsDoorRegisteredWithSystem(data.doorHash) then
                                ...
                                if data.locked then
                                    ...
                                    if data.doorType == "sliding" or data.doorType == "garage" then
                                        DoorSystemSetAutomaticDistance(data.doorHash, 0.0, false, false)
                                    end
                                else
                                    DoorSystemSetDoorState(data.doorHash, 0, false, false)
                                    if data.doorType == "sliding" or data.doorType == "garage" then
                                        DoorSystemSetAutomaticDistance(data.doorHash, 30.0, false, false)
                                        DoorSystemSetAutomaticRate(data.doorHash, data.doorRate or 1.0, false, false)
                                    end
                                end
                            end
                        end
	...
	while true do
		if current.doors then
			...
			for k, v in pairs(current.doors) do
				...
				if current.doorType == "doublesliding" or (current.doorType == "double" and current.autoOpen) then
					DoorSystemSetAutomaticRate(v.doorHash, v.doorRate or current.doorRate or 1.0, false, false)
			...
			if current.doorType == "custom" then
				playSound(current, src, enableSounds)
				return
			end
...
-- 测试后山双开门开合动画测试指令
RegisterCommand('testcartelgate', function()
	local door = Config.DoorList['cartel_gate_back']
	if not door or not door.doors then 
		QBCore.Functions.Notify("没有找到后山大门配置！", "error")
		return 
	end
	
	QBCore.Functions.Notify("开始执行后山大门开合动画测试...", "primary")
	
	-- 1. 临时解锁大门物理限制，允许旋转
	for k, v in pairs(door.doors) do
		if IsDoorRegisteredWithSystem(v.doorHash) then
			DoorSystemSetDoorState(v.doorHash, 0, false, false)
		end
	end
	
	-- 2. 模拟渐变角度平滑开启：从 0.0 到 1.0 (利用左/右模型原生镜像对称开启)
	CreateThread(function()
		for ratio = 0, 100, 2 do
			local floatRatio = ratio / 100.0
			for k, v in pairs(door.doors) do
				if IsDoorRegisteredWithSystem(v.doorHash) then
					DoorSystemSetOpenRatio(v.doorHash, floatRatio, true, true)
				end
			end
			Wait(30)
		end
		
		QBCore.Functions.Notify("大门已完全开启，停顿 3 秒...", "success")
		Wait(3000)
		
		QBCore.Functions.Notify("开始平滑关闭大门...", "primary")
		for ratio = 100, 0, -2 do
			local floatRatio = ratio / 100.0
			for k, v in pairs(door.doors) do
				if IsDoorRegisteredWithSystem(v.doorHash) then
					DoorSystemSetOpenRatio(v.doorHash, floatRatio, true, true)
		...
		-- 3. 测试完毕后重新设为物理锁死，防止穿模
		for k, v in pairs(door.doors) do
			if IsDoorRegisteredWithSystem(v.doorHash) then
				DoorSystemSetDoorState(v.doorHash, 1, false, false)
			end
		end
		QBCore.Functions.Notify("测试完成，大门已重新锁死！", "success")
	end)
end, false)