...
AddEventHandler('playerDropped', function(reason)
    ...
    Player.Functions.Save(true) -- 强制最后一次落盘，保障数据绝对安全
...
RegisterNetEvent('QBCore:UpdatePlayer', function(multiplier)
    local src = source
    ...
    if multiplier > 3.0 then multiplier = 3.0 end

    local newHunger = Player.PlayerData.metadata['hunger'] - (QBCore.Config.Player.HungerRate * multiplier)
    local newThirst = Player.PlayerData.metadata['thirst'] - (QBCore.Config.Player.ThirstRate * multiplier)
    if newHunger <= 0 then
        newHunger = 0
    end
    if newThirst <= 0 then
