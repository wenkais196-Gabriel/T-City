...
local function SetBedCam()
    ...
    -- Freeze player immediately to prevent falling through map before collisions load
    FreezeEntityPosition(player, true)

    local x, y, z = bedOccupyingData.coords.x, bedOccupyingData.coords.y, bedOccupyingData.coords.z
    local w = bedOccupyingData.coords.w

    if IsPedDeadOrDying(player) then
        NetworkResurrectLocalPlayer(x, y, z + 0.02, w, true, false)
    else
        SetEntityCoords(player, x, y, z + 0.02)
        SetEntityHeading(player, w)
    end

    -- Force load collision at the target hospital bed coordinates
    RequestCollisionAtCoord(x, y, z)
    local time = GetGameTimer()
    while not HasCollisionLoadedAroundEntity(player) and (GetGameTimer() - time) < 5000 do
        Wait(10)
    end
    Wait(500) -- Additional buffer to ensure full MLO map loading

    bedObject = GetClosestObjectOfType(x, y, z, 1.0, bedOccupyingData.model, false, false, false)
