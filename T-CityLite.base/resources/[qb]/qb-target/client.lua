	...
	while true do
		...
		if result ~= 1 then
			...
			if entityHit and entityHit ~= 0 then
				pcall(function()
					exists = DoesEntityExist(entityHit)
				end)
			end
			...
			if flag == 30 and entityHit and entityHit ~= 0 then
				local hasLos = false
				pcall(function()
					hasLos = HasEntityClearLosToEntity(entityHit, playerPed, 7)
				end)
				entityHit = hasLos and entityHit or 0
			end
...
local function EnableTarget()
	...
	if LocalPlayer.state['isDead'] or LocalPlayer.state['inLaststand'] then return end
