...
RegisterNetEvent('qb-inventory:client:openInventory', function(items, other)
    if not PlayerData or not PlayerData.metadata or PlayerData.metadata['isdead'] or PlayerData.metadata['inlaststand'] then return end
...
for i = 1, 5 do
    RegisterCommand('slot_' .. i, function()
        if not PlayerData or not PlayerData.metadata or PlayerData.metadata['isdead'] or PlayerData.metadata['inlaststand'] or LocalPlayer.state.inv_busy then return end
...
-- Disable original GTA weapon drops, weapon wheels, and native cash/bank HUD
CreateThread(function()
    while true do
        -- Disable GTA weapon wheel and weapon selection hotkeys (to prevent wheel from flashing)
        DisableControlAction(0, 37, true)  -- INPUT_SELECT_WEAPON (Tab)
        DisableControlAction(0, 157, true) -- INPUT_SELECT_WEAPON_UNARMED (1)
        DisableControlAction(0, 158, true) -- INPUT_SELECT_WEAPON_PISTOL (2)
        DisableControlAction(0, 159, true) -- INPUT_SELECT_WEAPON_SMG (3)
        DisableControlAction(0, 160, true) -- INPUT_SELECT_WEAPON_SHOTGUN (4)
        DisableControlAction(0, 161, true) -- INPUT_SELECT_WEAPON_RIFLE (5)
        DisableControlAction(0, 162, true) -- INPUT_SELECT_WEAPON_SNIPER (6)
        DisableControlAction(0, 163, true) -- INPUT_SELECT_WEAPON_HEAVY (7)
        DisableControlAction(0, 164, true) -- INPUT_SELECT_WEAPON_THROWABLE (8)
        DisableControlAction(0, 165, true) -- INPUT_SELECT_WEAPON_SPECIAL (9)

        -- Disable vehicle weapon/ammo rewards
        DisablePlayerVehicleRewards(PlayerId())

        -- Hide original GTA cash, bank, and cash change HUD components
        HideHudComponentThisFrame(3)  -- HUD_CASH
        HideHudComponentThisFrame(4)  -- HUD_MP_CASH (Bank)
...
-- Regularly disable weapon drops for all spawned NPC peds
CreateThread(function()
    while true do
