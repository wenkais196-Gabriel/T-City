...
local function DebugPrint(msg)
    if QBConfig.Custom.General.EnableDebug then
        print(('[custom-security] %s'):format(msg))
    end
end
...
AddEventHandler('playerDropped', function(reason)
    ...
    if isDead or isWounded or isHandcuffed then
        ...
        -- 组装 Discord 安全审计嵌入数据 (使用 custom-logs 导出)
        ...
        exports['custom-logs']:LogSecurity("战斗逃脱警报 (Combat Logging)", alertText, 13631488) -- 红色警报
    end
end)
...
AddEventHandler('QBCore:Server:OnMoneyChange', function(src, moneytype, amount, action, reason)
    ...
    if amount > maxLimit then
        ...
        if not isadmin then
            -- 非管理员玩家触发巨额变动，立刻引发安全审计警报 (使用 custom-logs 导出)
            exports['custom-logs']:LogSecurity("巨额资产异常流入", alertText, 16711680) -- 红色
        else
            -- 管理员变动仅作为系统备案审计
            exports['custom-logs']:LogGeneric("管理员划账审计", alertText, 10079487) -- 蓝紫色
        end
    end
end)
...
AddEventHandler('QBCore:Server:OnGangUpdate', function(src, gang)
    ...
    exports['custom-logs']:LogGeneric("帮派关系更新审计", text, 16738657) -- 粉橙色
end)
...
CreateThread(function()
    ...
    if origAttemptPurchase then
        ...
        print("[custom-security][WARNING] 无法找到 `qb-inventory:server:attemptPurchase` 注册信息，跳过劫持。")
    end
        ...
        QBCore.ServerCallbacks['QBCore:Server:SpawnVehicle'] = function(source, cb, model, coords, warp)
            ...
            if QBConfig.Custom.Security.CheckVehicleSpawn then
                ...
                if not isadmin and not isJobAuthorized then
                    ...
                    exports['custom-logs']:LogSecurity("拦截非法刷车企图", text, 16711680)
        ...
        QBCore.ServerCallbacks['QBCore:Server:CreateVehicle'] = function(source, cb, model, coords, warp)
            ...
            if QBConfig.Custom.Security.CheckVehicleSpawn then
                ...
                if not isadmin and not isJobAuthorized then
                    ...
                    exports['custom-logs']:LogSecurity("拦截非法长距车企图", text, 16711680)
                    cb(false)
                    return
                end
