...
local function DebugPrint(msg)
    if QBConfig.Custom.General.EnableDebug then
        print(('[custom-main][crime] %s'):format(msg))
    end
end
...
local function CheckStoreRobbery(src, registerId, isDone)
    ...
    if diffPlayer < 10 then -- 防客户端多发包瞬间触发
        ...
        exports['custom-main']:LogSecurity("商店抢劫高频调用", alertText)
        return false
    end
    ...
    if isDone then
        ...
        exports['custom-main']:LogGeneric("商店抢劫成功", logText, 65280) -- 绿色
    end
...
local function CheckHouseRobbery(src, houseId, cabinId)
    ...
    if Cooldowns.HouseRobberyFurniture[furnitureKey] then
        ...
        exports['custom-main']:LogSecurity("房屋重复搜刮拦截", alertText)
        return false
    end
    ...
    exports['custom-main']:LogGeneric("房屋搜刮审计", logText, 7434190)
...
local function CheckDrugs(src)
    ...
    exports['custom-main']:LogGeneric("毒品交易完成", logText, 4289797)
