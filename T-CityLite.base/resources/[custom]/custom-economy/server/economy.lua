...
local function DebugPrint(msg)
    if QBConfig.Custom.General.EnableDebug then
        print(('[custom-economy] %s'):format(msg))
    end
end
...
local function SaveEconomySnapshot()
    local data = {
        RewardScale = EconomyState.RewardScale,
        PriceScale = EconomyState.PriceScale,
        TotalEarned = EconomyState.TotalEarned,
        TotalSpent = EconomyState.TotalSpent,
        Events = EconomyState.Events,
        SavedAt = os.time()
    }
    local success = SaveResourceFile(GetCurrentResourceName(), "data/economy_state.json", json.encode(data), -1)
    if success then
        DebugPrint("Economy state snapshot saved successfully.")
    else
        DebugPrint("Failed to save economy state snapshot.")
    end
end

local function LoadEconomySnapshot()
    local content = LoadResourceFile(GetCurrentResourceName(), "data/economy_state.json")
    if content then
        local data = json.decode(content)
        if data then
            EconomyState.RewardScale = data.RewardScale or EconomyState.RewardScale
            EconomyState.PriceScale = data.PriceScale or EconomyState.PriceScale
            EconomyState.TotalEarned = data.TotalEarned or 0
            EconomyState.TotalSpent = data.TotalSpent or 0
            EconomyState.Events = data.Events or 0
            print(("[custom-economy] Successfully restored economy state from snapshot: RewardScale=%.2f, PriceScale=%.2f"):format(
                EconomyState.RewardScale, EconomyState.PriceScale
            ))
            return
        end
    end
    print("[custom-economy] No active economy snapshot found, using defaults.")
end
...
local function AddScaledMoney(src, moneytype, baseAmount, reason)
    ...
    if finalAmount >= 20000 then
        -- 绿金高额警报，高亮显示
        exports['custom-logs']:LogEconomy("大额资金发放", auditText, 65280) -- 绿色
    else
        exports['custom-logs']:LogEconomy("常规资金结算", auditText, 4289797) -- 翡翠绿
    end
...
CreateThread(function()
    ...
    while true do
        ...
        if math.abs(newScale - oldScale) > 0.0001 then
            ...
            exports['custom-logs']:LogEconomy("自适应经济倍率微调", auditText, 16776960) -- 黄色
        else
