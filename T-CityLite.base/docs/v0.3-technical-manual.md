...
## 🗄️ 9. DirtyFlush 脏数据批处理管道 (v0.6)

在 v0.6 中，`qb-core` 的 `Player.Save()` 已接入 DirtyFlush 管道，实现 Memory-Cache-First 状态机：

### 9.1 数据流
```
所有写操作 → 内存 (MarkDirty) → 定时批量刷盘 → DB
playerDropped → ForceFlush (直接 SQL，不经过 Save 避免循环)
txAdmin stop → FlushAll (全量)
resource stop → FlushAll (全量)
```

### 9.2 核心性能提升
- **非强制存盘**: `Player.Functions.Save()` 仅标记 `DirtyFlush.MarkDirty(citizenid, 'all')`，不执行 SQL
- **强制存盘**: `Player.Functions.Save(true)` → `DirtyFlush.ForceFlush(citizenid)` → 直接 MySQL.insert
- **hunger/thirst 优化**: `SetMetaData` 中饥饿/口渴不再标记 `IsDirty`，消除每 5 分钟全字段 UPDATE
- **IO 削减**: 100 人 × 每 5 分钟 100 次 SQL → 每 15 分钟 1 次批量写入，**IO -66%**

### 9.3 零回档保障
- 写失败不清理 dirty 标记 → 下次重试
- `playerDropped` → 立即 ForceFlush
- txAdmin 关闭/重启 → 双重信号检测 + 全量刷盘 + 2 秒等待兜底

### 9.4 关键文件
- `core-framework/cache/dirty_flush.lua` — DirtyFlush 状态机 + ForceFlush 直接 SQL
- `core-framework/services/persistence_manager.lua` — 持久化编排器 + 健康监控
- `qb-core/server/player.lua:526-570` — Save() 管道代理

---

## 🛡️ 10. Bus SecurityService 事件防火墙 (v0.6)

### 10.1 架构
`Bus.SecurityService` 作为独立 Service 注册到 Bus 总线，提供三大核心能力：

| 能力 | 方法 | 说明 |
|:-----|:-----|:-----|
| **输入清洗** | `SanitizeNumber`, `SanitizeString`, `SanitizeInput` | 类型安全 + 边界钳制 + 模式校验 |
| **阈值熔断** | `CheckSingleThreshold`, `CheckCumulativeThreshold` | 单笔上限 + 10 分钟累计窗口 |
| **Source 校验** | `ValidateSource`, `ValidateSourceWithJob`, `ValidateMoneyEvent`, `ValidateJobEvent`, `ValidateGangEvent`, `ValidateItemEvent` | 组合校验器 |

### 10.2 集成点
已在 `qb-core/server/player.lua` 中为以下函数添加 SecurityService 校验：
- `AddMoney` / `RemoveMoney` / `SetMoney` — 金额清洗 + 单笔阈值 + 累计熔断
- `SetJob` / `SetGang` — 名称清洗 + 等级钳制

---

## 🔒 11. F8 客户端控制台日志深度优化与防泄露机制 (F8 Console Logging Optimization & Information Leak Prevention)
