...
## 5. v0.6 DirtyFlush 批处理管道（异步优化下一阶段）

v0.2 解决了"单次 SQL 不阻塞主线程"的问题，v0.6 进一步解决了"每次变动都触发 SQL"的问题：

### 5.1 架构演进

```
v0.2: Save() → MySQL.insert(callback)  [异步不阻塞，但每次变动都写]
v0.6: Save() → DirtyFlush.MarkDirty → 定时批量落盘  [减少写入频率]
```

### 5.2 工作原理

1. `Player.Functions.Save()` 调用时，不再直接执行 MySQL.insert
2. 改为 `DirtyFlush.MarkDirty(citizenid, 'all')`，仅标记该玩家数据已变更
3. DirtyFlush 每 15 分钟遍历脏玩家池，对每个脏玩家调用 `ForceFlush`
4. `ForceFlush` 直接执行 MySQL.insert（带 `ON DUPLICATE KEY UPDATE`）
5. 写失败不清理 dirty 标记 → 下次自动重试

### 5.3 强制刷盘场景

- `playerDropped` → 立即 ForceFlush（防止数据丢失）
- `Player.Functions.Save(true)` → 立即 ForceFlush
- txAdmin 关闭/重启 → 全量 FlushAll + 2 秒兜底
- `onResourceStop` → 全量 FlushAll

### 5.4 关键文件

| 文件 | 职责 |
|:-----|:-----|
| `core-framework/cache/dirty_flush.lua` | 脏玩家池 + ForceFlush 直接 SQL + 定时 Tick + 关机钩子 |
| `core-framework/services/persistence_manager.lua` | 持久化编排器 + 健康报告 |
| `qb-core/server/player.lua` | Save() 管道代理（DirtyFlush 优先，fallback 原始 SQL） |

### 5.5 兼容性

若 `core-framework` 未启动（`DirtyFlush == nil`），`Save()` 自动回退到 v0.2 的原始异步 SQL 逻辑，完全向后兼容。
